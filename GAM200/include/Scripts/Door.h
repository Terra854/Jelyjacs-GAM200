...
// Class Door, derived from the LogicScript class.
// This class defines the behavior of a portal in a game, allowing objects (like a player) to teleport between locations.
class Door : public LogicScript
{
public:
    // Constructor for the Door class.
    // @param name: A string representing the name of the Door instance.
    Door(std::string name);

    // Default destructor for the Door class.
    // It is marked as default to use the compiler-generated destructor.
    ~Door() = default;

    // Start method, called when the Door script is first activated.
    ...
    // Shutdown method, called when the Door script is being deactivated or destroyed.
    // @param obj: A pointer to the Object that this script is attached to.
