...
void ChatBoxLine1::Update(Object* obj) {
    ...
    trans->Scale.x = find_width(text->text, text->fontType);
    ...
    float top_left = chatbox_trans->Position.x - chatbox_trans->Scale.x*0.8f / 2.0f;
    trans->Position.x = top_left + trans->Scale.x / 2.0f;
